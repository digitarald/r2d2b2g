...
(function touchEventHandler() {
  ...
  let TouchEventHandler = {
    ...
    sendTouchEvent: function teh_sendTouchEvent(evt, target, name) {
      let doc = target.ownerDocument;
      let touchEvent = doc.createEvent('touchevent');
      console.log('createTouch');
      let point = doc.createTouch(window, target, 0,
                                     evt.pageX, evt.pageY,
                                     evt.screenX, evt.screenY,
                                     evt.clientX, evt.clientY,
                                     1, 1, 0, 0);
      console.log('createTouchList');
      let touches = doc.createTouchList(point);
  ...
  window.addEventListener('ContentStart', function touchStart(evt) {
    ...
    console.log('############################# touch.js ContentStart');
    TouchEventHandler.start();
    console.log('############################# touch.js STARTED');

    Cu.import("resource://gre/modules/Services.jsm");
    console.log(Services.prefs.getIntPref('dom.w3c_touch_events.enabled'));
  });
})();
